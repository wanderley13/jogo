...
class EntityMediator:
    ...
    def __verify_collision_entity(ent1, ent2):
        ...
        if valid_interaction:
            if (ent1.rect.right >= ent2.rect.left and ent1.rect.left <= ent2.rect.right and
                ...
                ent1.u_dano = ent2.name

    @staticmethod
    def __g_score(enemy: Enemy, entity_list: list[Entity]):
        if enemy.u_dano == 'Player1Shot1':
            for ent in entity_list:
                if ent.name == 'Player1':
                    ent.score += enemy.score
        elif enemy.u_dano == 'Player1Shot':
            for ent in entity_list:
                if ent.name == 'Player1':
                    ent.score += enemy.score
    ...
    def verify_health(entity_list: list[Entity]):
        for ent in entity_list:
            if ent.health <= 0:
                if isinstance(ent, Enemy):
                    EntityMediator.__g_score(ent, entity_list)
